...
fn find_cluster_chunks(hash_size: usize, origin: Point<i32>, rectangle_width: i32, num_cluster_chunks: usize, hashmap: &OpenHashMap, glass_hash: usize, illegal_chunks: &OpenHashMap, min_search: usize) -> QuestionableBool {
    ...
    save_hopper_setup_mcfunction(&best_cluster_chunks).ok()?;
    println!("Hopper setup mcfunction saved to hopper_setup.mcfunction");

    return YES;
}

fn save_hopper_setup_mcfunction(cluster_chunks: &Vec<Pos>) -> std::io::Result<()> {
    ...
    std::fs::write("hopper_setup.mcfunction", file)?;
